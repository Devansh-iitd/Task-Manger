...
const FriendsCard = (props) => {
  ...
  return (
    <div className=" w-72 h-52 rounded-xl bg-white m-9">
      <div
        className=" w-32 h-6 rounded mt-4 ml-4"
        style={{ backgroundColor: '#A3E5D1' }}
      >
        <a className=" text-base font-medium text-black font-sans my-auto mx-auto">
          {name}
        </a>
      </div>
      ...
      {!requester && !friends ? (
        <button
          type="button"
          className="bg-gradient-to-r from-green-600 to-green-300 rounded-md text-white    h-6 hover:cursor-pointer ml-10 mb-4 px-1"
          value={name}
          onClick={accept}
        >
          Accept
